...
| Endpoints | one labelled text `Input` per known endpoint (`Label` + `Input`) — `MacIp` and `AppManagerUrl` (REQ-FN-028); URL fields carry a hint line under the input, and those that allow it also render an opt-in `Switch` "Trust a self-signed certificate for this endpoint" | `TrSetupSettings.Endpoints` values + `TrSetupSettings.TrustedSelfSignedEndpoints` | per-kind validation: `Address` → IP/hostname, `Url` → absolute `http(s)://`; blank always allowed (falls back to the profile default). Never a secret field (ADR-008 — secrets are presence-only, never entered or shown here). The TLS-trust switch defaults **off** and is honoured only for a user-configured URL |
