...
open "src/TrSetup/bin/Release/net10.0-maccatalyst/TrSetup.app"
```
...
- Output bundle: **`src/TrSetup/bin/Release/net10.0-maccatalyst/TrSetup.app`** — note there is **no RID sub-folder** in the path. Release Catalyst defaults to `RuntimeIdentifiers=maccatalyst-x64;maccatalyst-arm64`, so the build runs two per-RID *inner* builds and then merges them (lipo) into one **universal** bundle at the path above. ⚠ **The `maccatalyst-arm64/` and `maccatalyst-x64/` sub-folders are build intermediates and their `Contents/Resources/wwwroot` is deliberately EMPTY** — MAUI's `DisableResizetizer` gate (`Microsoft.Maui.Resizetizer.After.targets`, `_IsMultiRidBuild`) skips asset staging for inner builds, because only the outer build stages assets. Launching an inner bundle shows *"There is no content at "*; that is not a defect, it is the wrong artifact. This path trap cost two investigations (2026-07-21) — always launch the RID-less bundle. **Debug differs**: it builds a single RID, so there the deliverable *is* `maccatalyst-arm64/TrSetup.app` and its wwwroot is populated. (If you ever want the per-RID path to be the real output, `dotnet build -c Release -p:RuntimeIdentifier=maccatalyst-arm64` does that — at the cost of the universal binary. Not recommended for distribution.)
